...
/*   Updated: 2019/04/28 20:50:09 by fsmith           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void*		fr_thread_mandelbrot(void* thread_data)
{
	...
	while (x < data->end_x)
	{
		...
		while (y < WINDOW_H)
		{
			...
			n_r = 0;
			n_i = 0;
			c_r = (double)(x - WINDOW_W / 2 + data->frc->offset_x) / data->frc->scale;
			c_i = (double)(y - WINDOW_H / 2 + data->frc->offset_y) / data->frc->scale;
			temp_x = x;
			temp_y = y;
